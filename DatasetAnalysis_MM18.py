...
if compute_furthest_mot:
    ...
    for prediction_window in [0.2, 0.5, 1.0, 2.0, 5.0, 15.0]:
        ...
        aver = round(sum(max_velocities) / len(max_velocities),1)
        n, bins, patches = plt.hist(max_velocities, bins=np.arange(0, 360, 0.1), density=True, histtype='step',
                                    cumulative=True,
                                    label="average angel: " + str(aver) + "---> in " + str(prediction_window) + 's')
        #n, bins, patches = plt.hist(max_velocities,bins=np.arange(0, 360, 0.1), density=True, histtype='step', cumulative=True, label=str(prediction_window) + 's')
        # hist, bin_edges = np.histogram(max_velocities, bins=np.arange(0, 360), density=True)
        # for i in range(len(hist)):
        #     print (bin_edges[i], hist[i])

    plt.xlabel('Motion angel from t to t+T [Degrees]')
    ...
    plt.savefig('./MM18.eps', format='eps')
    plt.show()
