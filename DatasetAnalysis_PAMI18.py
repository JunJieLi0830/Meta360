...
def get_trace(video, user):
    filename = './PAMI18/trace_%s_%s.csv' % (user, video)
...
for time_t in [0.2, 0.5, 1, 2, 5, 15]:
    ...
    aver = round(sum(average_velocities) / len(average_velocities), 1)

    # Compute the CDF
    n, bins, patches = plt.hist(average_velocities, bins=np.arange(0, 360, 0.1), density=True, histtype='step',
                                cumulative=True, label="average angle: " + str(aver) + "---> in " + str(time_t) + 's')
    #n, bins, patches = plt.hist(average_velocities, bins=np.arange(0, 360, 0.1), density=True, histtype='step', cumulative=True, label=str(time_t)+'s')
    # hist, bin_edges = np.histogram(average_velocities, bins=np.arange(0, 360), density=True)
    # for i in range(len(hist)):
    #     print bin_edges[i], hist[i]
plt.xlabel('Motion angle from t to t+T [Degrees]')
...
plt.savefig('./PAMI.eps', format='eps')
plt.show()
