...
for m_window in [5, 12, 25, 50, 125, 375]:
    ...
    n, bins, patches = plt.hist(average_velocities, bins=np.arange(0, 360, 0.1), density=True, histtype='step',
                                cumulative=True, label="average angle: " + str(aver) + "---> in " + str(round(m_window*1.0/25.0,1)) + 's')

    #n, bins, patches = plt.hist(average_velocities, bins=np.arange(0, 360, 0.1), density=True, histtype='step', cumulative=True, label=str(m_window*1.0/25.0)+'s')
...
plt.savefig('./cvpr.eps', format='eps')
plt.show()
