...
for time_t in [0.2, 0.5, 1, 2, 5, 15]:
    ...
    # Compute the CDF
    n, bins, patches = plt.hist(average_velocities, bins=np.arange(0, 360 , 0.1), density=True, histtype='step', cumulative=True, label="average angle: "+str(aver)+"---> in "+ str(time_t)+'s')

#plt.xlabel('Motion angel from last position (t -> t+T) [Degrees]')
...
plt.savefig('./nossdav.eps', format='eps')
plt.show()
